...
function checkResult() {
    if (slots && result && spinButton) {
        ...
        if (slotSymbols[0] === slotSymbols[1] && slotSymbols[1] === slotSymbols[2]) {
            ...
            score = 100;
        } else if (slotSymbols[0] === slotSymbols[1] || slotSymbols[1] === slotSymbols[2] || slotSymbols[0] === slotSymbols[2]) {
...
async function sendScore(score: number) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Server responded with ${response.status}: ${response.statusText}. Details: ${JSON.stringify(errorData)}`);
        }
